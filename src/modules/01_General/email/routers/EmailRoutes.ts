...
import { User, EmailController, UserRepository } from '@index/modules/01_General/email/index';
...
class EmailRoutes extends GenericRoutes {
    ...
    protected initializeRoutes() {
         ...
         *****************************/
         ...
         // Route for sending an email to multiple users based on specified filters
         this.router.post(`${this.getRouterName()}/send_email_by_weekly_sales`, async (req: Request, res: Response) => {
          
            // Create a request handler object with the necessary data
            const requestHandler: RequestHandler = new RequestHandlerBuilder(res, req)
                .setMethod("send_email_by_weekly_sales") // Set the method to be executed
                .isValidateRole("SEND_MAIL") // Validate the role of the user
                .isLogicalDelete() // Set the logical delete flag
                .build(); // Build the request handler object

            // Execute the sendMailByFilters method of the EmailController with the request handler object
            (this.getController() as EmailController).sendMailByWeeklySales(requestHandler);
        });



        // Route for sending an email to multiple users based on specified filters
        this.router.post(`${this.getRouterName()}/send_email_by_weekly_lead`, async (req: Request, res: Response) => {
          
            // Create a request handler object with the necessary data
            const requestHandler: RequestHandler = new RequestHandlerBuilder(res, req)
                .setMethod("send_email_by_weekly_lead") // Set the method to be executed
                .isValidateRole("SEND_MAIL") // Validate the role of the user
                .isLogicalDelete() // Set the logical delete flag
                .build(); // Build the request handler object

            // Execute the sendMailByFilters method of the EmailController with the request handler object
            (this.getController() as EmailController).sendMailByWeeklyLeads(requestHandler);
        });



         // Route for sending an email to multiple users based on specified filters
         this.router.post(`${this.getRouterName()}/send_email_by_weekly_free_trial`, async (req: Request, res: Response) => {
          
            // Create a request handler object with the necessary data
            const requestHandler: RequestHandler = new RequestHandlerBuilder(res, req)
                .setMethod("send_email_by_weekly_free_trial") // Set the method to be executed
                .isValidateRole("SEND_MAIL") // Validate the role of the user
                .isLogicalDelete() // Set the logical delete flag
                .build(); // Build the request handler object

            // Execute the sendMailByFilters method of the EmailController with the request handler object
            (this.getController() as EmailController).sendMailByWeeklyFreeTrial(requestHandler);
        });



          // Route for sending an email to multiple users based on specified filters
          this.router.post(`${this.getRouterName()}/send_email_by_weekly_cancellations`, async (req: Request, res: Response) => {
            
            // Create a request handler object with the necessary data
            const requestHandler: RequestHandler = new RequestHandlerBuilder(res, req)
                .setMethod("send_email_by_weekly_cancellations") // Set the method to be executed
                .isValidateRole("SEND_MAIL") // Validate the role of the user
                .isLogicalDelete() // Set the logical delete flag
                .build(); // Build the request handler object

            // Execute the sendMailByFilters method of the EmailController with the request handler object
            (this.getController() as EmailController).sendMailByWeeklyCancellations(requestHandler);
        });
    }
}
